...
def test_model(model, test_loader, class_names, dataset_name, save_dir='models/resnet/results'):
    """
    测试模型并保存结果
...
if __name__ == '__main__':
    ...
    # 1. 选择数据集
    dataset_name = 'oxford_pets'  # 可选: 'oxford_pets', 'stanford_dogs', 'cats_vs_dogs', 'dogs77'
    ...
    model = ResNet34(Residual, num_classes=num_classes)
    model_create_time = time.time() - model_create_start
    print(f"模型创建完成，耗时: {model_create_time:.2f}秒")
    
    # 6. 加载训练好的模型权重
    weight_load_start = time.time()
    model_weight_path = 'models/resnet/weights/resnet34_best_oxford_pets_20260308_201051.pth'  # 替换为实际的权重文件路径
    if os.path.exists(model_weight_path):
        model.load_state_dict(torch.load(model_weight_path, weights_only=True))
        print(f"已加载模型权重: {model_weight_path}")
    else:
        weights_dir = 'models/resnet/weights'
        if os.path.exists(weights_dir):
            weight_files = [f for f in os.listdir(weights_dir) if f.endswith('.pth') and 'resnet34' in f]
            if weight_files:
    ...
    print(f"文件位置: models/resnet/results/")
    print(f"{'='*60}\n")
