...
from model import ResNet34 #加深了深度，提升了精度，用于突出性能上的差别。
...
class TeeLogger:
    ...
    def __init__(self, log_file=None):
        # 默认保存到 models/resnet/results/training_{timestamp}.log
        if log_file is None:
            timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
            log_dir = 'models/resnet/results'
...
def train_model_process(model, train_loader, val_loader, num_epochs=10, dataset_name=None):
    ...
    optimizer = torch.optim.SGD(
        model.parameters(),
        lr=0.01 * (accumulation_steps ** 0.5),  # 调整学习率
        momentum=0.9,
        weight_decay=5e-4
    )
    ...
    os.makedirs('models/resnet/weights', exist_ok=True)
    ...
    # 保存模型
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    torch.save(best_model_wts, f'models/resnet/weights/resnet34_best_{dataset_name}_{timestamp}.pth')
...
def matplot_acc_loss(train_process, save_plot=True):
    ...
    if save_plot:
        timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
        plot_dir = 'models/resnet/results'
...
if __name__ == '__main__':
    ...
    # ===== 启用日志记录 =====
    # 创建 results 目录
    os.makedirs('models/resnet/results', exist_ok=True)
    ...
    try:
        # 选择数据集
        dataset_name = 'dogs77'  # 或 'oxford_pets', 'stanford_dogs','cats_vs_dogs', 'doges77'
        ...
        resnet = ResNet34(Residual, num_classes=num_classes)
        ...
        train_process = train_model_process(resnet, train_loader, val_loader, num_epochs=80, dataset_name=dataset_name)
