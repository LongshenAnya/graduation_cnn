...
def test_model(model, test_loader, class_names, dataset_name, save_dir='models/mobilenet/results'):
    """
    测试模型并保存结果
...
if __name__ == '__main__':
    ...
    # 1. 选择数据集
    dataset_name = 'cats_vs_dogs'  # 或 'oxford_pets', 'stanford_dogs', 'cats_vs_dogs', 'dogs77'
    ...
    model_weight_path = 'models/mobilenet/weights/mobilenetv1_best_cats_vs_dogs_20260303_100635.pth'
    if os.path.exists(model_weight_path):
        model.load_state_dict(torch.load(model_weight_path, weights_only=True))
        print(f"已加载模型权重: {model_weight_path}")
    else:
        weights_dir = 'models/mobilenet/weights'
        if os.path.exists(weights_dir):
            weight_files = [f for f in os.listdir(weights_dir) if f.endswith('.pth') and 'mobilenetv1' in f]
            if weight_files:
    ...
    print(f"文件位置: models/mobilenet/results/")
    print(f"{'='*60}\n")
