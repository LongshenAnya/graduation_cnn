"""
宠物图像数据集加载器
支持预处理后的数据集: cats_vs_dogs, oxford_pets, stanford_dogs, doges77

本文件负责：
- 读取已处理数据集的 images_info.csv 和 labels.csv
- 构建 PyTorch Dataset
- 提供训练/验证/测试 DataLoader
- 在训练集上应用数据增强
"""
...
class PetDataset(Dataset):
    ...
    def _get_transform(self):
        ...
        训练集使用数据增强，验证/测试集只使用基础预处理。
        """
        # 基础变换：统一调整大小、转换为 Tensor、按 ImageNet 统计量归一化
        base_transform = [
        ...
        if self.augment:
            # === 数据增强 START (仅训练集有效) ===
            # 训练集增强方案：
            # - RandomResizedCrop: 随机裁剪并缩放，增强尺度和构图鲁棒性
            # - RandomHorizontalFlip: 随机水平翻转，提高左右翻转不变性
            # - RandomRotation: 小角度旋转，增强旋转不变性
            # - ColorJitter: 随机颜色扰动，增强亮度/对比度/饱和度/色相变化
            # - RandomAffine: 随机平移，增强位置变换鲁棒性
            transform_list = [
